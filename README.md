...
4. Улучшить отображение для удобства просмотра в /work-orders: добавить фильтрацию, поиск, перетаскивание элементов для их закрытия и т.д.
5. Редактирование пользователей: если сменят место работы, уволятся, сменят ФИО и т.д.

Дополнительно по фронту (out of scope): экраны воронки (записи/визиты) и кассы (API уже есть), мобильный сценарий для рабочего на посту.
...
2. `002_seed_demo` (+ `003_expand_wo_seed`) — демо-данные: филиалы, роли, визиты и **~19 ЗН** (по 1–3 на каждый статус для канбана)
...
| --------- | ----------------------------------------------------------------- | --------------------------- |
| 1         | [director@sto.local](mailto:director@sto.local)                   | director (вся сеть)         |
| 2         | [manager.lenina@sto.local](mailto:manager.lenina@sto.local)       | branch_manager (СТО Ленина) |
| 3         | [manager.south@sto.local](mailto:manager.south@sto.local)         | branch_manager (СТО Юг)     |
| 4         | [worker.suspension@sto.local](mailto:worker.suspension@sto.local) | worker                      |
| 5         | [worker.paint@sto.local](mailto:worker.paint@sto.local)           | worker                      |
| 6         | [worker.south@sto.local](mailto:worker.south@sto.local)           | worker                      |
...
1. Запустите API и откройте [http://localhost:8000/docs](http://localhost:8000/docs)
